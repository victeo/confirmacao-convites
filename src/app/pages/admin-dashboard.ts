...
@Component({
  ...
  template: `
    <div class="p-6 md:p-12 max-w-7xl mx-auto animate-in fade-in duration-700">
      ...
      <section *ngIf="showAddForm" class="mb-12 animate-in zoom-in duration-500">
        <div class="bg-white p-8 rounded-3xl border border-primary/20 shadow-xl relative overflow-hidden">
          ...
          <div class="mt-10 p-6 bg-surface-container/50 rounded-2xl border border-outline-variant/20">
            ...
            <div class="flex flex-wrap gap-3">
              <div *ngFor="let name of dependentsList; let i = index" class="pl-5 pr-3 py-2 bg-primary/10 text-primary border border-primary/20 rounded-full flex items-center gap-3 font-semibold shadow-sm">
                {{ name }}
                <button (click)="removeDependent(i)" class="w-6 h-6 rounded-full bg-primary/20 text-primary hover:bg-primary hover:text-on-primary transition-colors flex items-center justify-center font-bold text-xs leading-none">×</button>
              </div>
              <p *ngIf="dependentsList.length === 0" class="text-sm text-outline italic py-2">Nenhum dependente adicionado ainda.</p>
            </div>
          </div>
      ...
      <section class="bg-white rounded-3xl border border-outline-variant/30 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-1000">
        <div class="p-8 border-b border-outline-variant/30 flex justify-between items-center bg-surface-container/30">
          ...
          <button *ngIf="!showAddForm" (click)="showAddForm = true" class="px-6 py-3 bg-primary text-on-primary font-bold rounded-xl active:scale-95 transition-all shadow-md shadow-primary/10">Novo Convite +</button>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-outline-variant/20">
              <tr *ngFor="let group of filteredGroups()" class="hover:bg-surface-container/20 transition-colors">
                <td class="p-6">
                ...
                <td class="p-6">
                  <div class="flex flex-wrap gap-2">
                    <!-- Convidados Pré-Registrados -->
                    <div *ngFor="let g of group.preRegisteredGuests" 
                         class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border"
                    ...
                    <!-- Extras -->
                    <div *ngFor="let g of group.extraGuests" 
                         class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border border-primary/30 bg-primary/5 text-primary">
  ...
  styles: [`
    ...
    button:not(.active-filter) { color: #615e57; }
...
export class AdminDashboardComponent implements OnDestroy {
  ...
  // Signal computado que filtra a lista automaticamente
  filteredGroups = computed(() => {
    ...
    // Um convite é considerado "Confirmado" se pelo menos um membro do grupo confirmou.
    // Ou podemos filtrar se o titular confirmou. Vamos usar a lógica de "Contém pelo menos um":
    return list.filter(group => {
      const allMembers = [...group.preRegisteredGuests, ...group.extraGuests];
      return allMembers.some(member => member.status === filter);
    });
  });
  ...
  totalGuests() { return this.groups().reduce((acc, g) => acc + g.preRegisteredGuests.length + g.extraGuests.length, 0); }
  totalConfirmed() { return this.groups().reduce((acc, group) => {
  ...
  addDependent() {
    if (this.newDependentName.trim()) {
      this.dependentsList.push(this.newDependentName.trim());
      this.newDependentName = '';
    }
  }

  removeDependent(index: number) { this.dependentsList.splice(index, 1); }
  ...
  editGroup(group: RSVPGroup) {
    ...
    this.dependentsList = group.preRegisteredGuests.filter(g => g.type === GuestType.DEPENDENT).map(g => g.name);
